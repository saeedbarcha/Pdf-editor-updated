...
function DraggableElement({ element, index, moveElement }) {
...
// function DraggableElement({ element, index, moveElement, setHeaderElements }) {
function Header({ onDrop, headerElements, setHeaderElements }) {
    ...
    let canvasBg = headerElements.length === 0 ? {
        width: "818px",
        minHeight: "138px",
        backgroundColor: "#fff",
    ...
    return (
        <div style={{ height: "auto", minHeight: `${bodyHeight}px`, width: "834px", background: "#F6F6F6", display: "flex", flexDirection: "column", alignContent: "center", borderRadius: "4px", padding: "16px 8px", lineHeight: "16.94px" }}>
            <p style={{ fontWeight: "600", fontSize: "14px", color: "#3A6B88" }}>Header</p>
            <div ref={dropRef} style={canvasBg}>
                {headerElements.length === 0 ? (
                  ...
                  headerElements.map((element, index) => (
                        <DraggableElement key={index} index={index} element={element} moveElement={moveElement} setHeaderElements={setHeaderElements} />
                    ))
                )}
            </div>
        </div>
